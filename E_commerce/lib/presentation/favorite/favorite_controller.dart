...
import '../home_screen/models/home_models.dart';
...
class FavoriteController extends GetxController {
  ...
  void addToFavorite(Product item) {
    ...
    saveFavoriteStatus(item.id as int, true); // Save favorite status to storage
  }

  void removeFromFavorite(Product item) {
    favoriteItems.remove(item);
    saveFavorites(); // Save to storage after removing from favorites
    saveFavoriteStatus(item.id as int, false); // Save favorite status to storage
  }
